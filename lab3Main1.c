...
Led1State led1State;
Led2State led2State;
uint16_t clockPeriodArray[4]={30000, 375000, 18750, 9375};
...
void configTimer(uint32_t timer, uint16_t clockPeriod, uint16_t clockDivider);
...
int main(void)
{
    ...
    led1State = OFF1;
    led2State = OFF2;
    configTimer(TIMER, clockPeriodArray[stateCounter0], clockDividerArray[stateCounter1]);
    MAP_Interrupt_enableSleepOnIsrExit(); //Add this statements
...
*/
    ...
    Timer_A_registerInterrupt(timer, TIMER_A_CCRX_AND_OVERFLOW_INTERRUPT, toggleLEDs);
...
void pushButtons(void){
    ...
    if((GPIO_getInterruptStatus(PORT1, PIN1)) != 0)
    {
        ...
        //increments the period for the timer
        if(stateCounter0 == 3)
        {
            stateCounter0 = 0;
        }
